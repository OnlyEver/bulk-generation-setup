...
export async function populateQueue(sourceId: string) {
    ...
    try {
        ...
        if (source) {
            ...
            const aiCards = source._ai_cards.map((elem: { _id: ObjectId, position: number }) => elem._id);

            if (Array.isArray(generationInfo) && generationInfo.length > 0) {
                const lastBreadthRequest = lastWhere(generationInfo, (item) => item.req_type.type === 'breadth');
                ...
                if (lastBreadthRequest || sourceTaxonomy) {
                    if (lastBreadthRequest.req_type.n_reqs <= calculatedViewTime) {
                        _insertBreadthRequest((lastBreadthRequest.req_type?.n_reqs ?? 0) + 1);
                    } else {
    ...
    function _insertBreadthRequest(n_reqs: number) {
        documents.push({
            "_source": sourceId,
            "ctime": new Date(),
            "status": "created",
...
async function handleDepthRequest(
    ...
    try {
        ...
        const conceptTextArray = concepts.map((concept: { concept_text: string, reference: string }) => concept.concept_text);
        const factTextArray = facts.map((fact: { fact_text: string, reference: string }) => fact.fact_text);

        const bloomLevelCards = await cardCollection.aggregate([
        ...
        if (sourceTaxonomy.generate_cards.state) {
            ...
            for (let bloom = 1; bloom <= 5; bloom++) {
                ...
                const lastDepthRequest = lastWhere(generationInfo, (item) => item.req_type?.type === 'depth' && item.req_type?.bloom_level === bloom);

                if (lastDepthRequest) {
                    if ((lastDepthRequest.req_type?.n_reqs ?? 0) <= maxRequestsForBloom) {
                        let levelCards = [];
                        levelCards = bloomLevelCards.find((item) => item.level === bloom)?.cards || [];
                        ...
                        if (levelCards.length > 0) {
                            ...
                            if (levelConcepts.length > 0) {
                                const c = conceptTextArray.filter((concept: string) => !levelConcepts.includes(concept));
                                missingConcepts = [];
                                missingConcepts.push(...c);
                            }

                            if (levelFacts.length > 0) {
                                const f = factTextArray.filter((fact: string) => !levelFacts.includes(fact));
                                missingFacts = [];
                                missingFacts.push(...f);
                            }

                            if (missingConcepts.length > 0 || missingFacts.length > 0) {
                                const missingConceptsData = missingConcepts.map((concept: string) => { return concepts.find((e: any) => e.concept_text === concept) });
                                const missingFactsData = missingFacts.map((fact: string) => { return facts.find((e: any) => e.fact_text === fact) });
                                ...
                                documents.push({
                                    ...
                                    "request_type": {
                                        "type": "depth",
                                        "bloom_level": bloom,
                                        "n_reqs": (lastDepthRequest?.n_reqs ?? 0) + 1
                                    },
                                    "params": {
